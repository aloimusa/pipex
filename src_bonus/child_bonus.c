...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/14 18:45:12 by aloimusa          #+#    #+#             */
/*   Updated: 2025/09/14 18:45:13 by aloimusa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exit_error(char *msg, void *obj, int action, int code)
{
	if (action == OPEN_FAIL)
	{
		if (errno == EPERM || errno == EACCES)
			exit_error("%s: permission denied\n", obj, NO, 1);
		else if (errno == ENOSPC)
			exit_error("%s: no space left on device\n", obj, NO, 0);
		else if (errno == ENOENT)
			exit_error("%s: no such file\n", obj, NO, 126);
		else if (errno == EISDIR)
			exit_error("%s: is a directory\n", obj, NO, 126);
		else
			exit_error("%s: undefined error opening file", obj, NO, 1);
	}
